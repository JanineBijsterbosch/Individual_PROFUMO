...
for s = 1:size(subs,2)
    ...
    map5 = subj.dtseries(:,5); map11 = subj.dtseries(:,11);
    other = nansum(subj.dtseries(:,setdiff(1:size(subj.dtseries,2),[5 11])),2);
    Iboth = find(map5>1 & map11>1 & other<0.1);
    I5 = find(map5>1 & map11<0.1 & other<0.1);
    I11 = find(map11>1 & map5<0.1 & other<0.1);
    Nvertices(s,:) = [length(I5) length(I11) length(Iboth)];
    clear other subj
    map_new(I5,s) = 1; map_new(I11,s) = 5; map_new(Iboth,s) = 10;
    ...
    if RunHMM
        if Iboth
            ...
            for n = 1:size(Scans,1)
                ...
                d = [mean(D.dtseries(I5,:))' mean(D.dtseries(I11,:))' mean(D.dtseries(Iboth,:))'];
                ...
                O_mix = D.dtseries(I5(randi(length(I5),floor(length(Iboth/2)),1)),:);
                O_mix = [O_mix; D.dtseries(I11(randi(length(I11),ceil(length(Iboth/2)),1)),:)];
                ...
                I7new = I5; I8new = I11;
                [~,I] = max([map5(Iboth) map11(Iboth)],[],2);
            ...
            save(sprintf('Results/HMM_functionally_different_output_subject%02d_%d.mat',s,subs(s)));
        end
    end   
end
...
ft_write_cifti('Results/Maps/Two_network_overlap_functionally_different',cifti,'parameter','dtseries')
clear cifti
...
ylabel('Gamma similarity'); xlabel('Number of states')
print(gcf,'Results/HMM_functionally_different_Gamma_similarity','-dpng','-r300');
...
ylabel('Absolute agreement with original'); xlabel('Number of states')
print(gcf,'Results/HMM_functionally_different_Means_similarity_A1','-dpng','-r300');
...
ylabel('Absolute agreement with original','FontSize',14); title('HMM state mean similarity (based on 2 states)','FontSize',14)
print(gcf,'Results/HMM_functionally_different_Means_similarity_summary','-dpng','-r300');
...
print(gcf,'Results/TS_similarity_functionally_different','-dpng','-r300');



% Align means across participants
k = 3;
for s = setdiff(2:length(subs),find(Nvertices(:,3)==0))
...
M_original = []; M_switch50 = []; M_switch25 = []; M_switch10 = []; M_max = []; M_linear = []; M_nonlinear = []; M_mix = []; M_gradient = [];
for s = setdiff(1:length(subs),find(Nvertices(:,3)==0))
...
print(gcf,'Results/HMM_functionally_different_mean_original_linear_k3','-dpng','-r300');
...
print(gcf,'Results/HMM_functionally_different_mean_original_linear_k2','-dpng','-r300');
...
% Save output
clear r rep1 rep2 reps s Scans tp v window D d assign i I I5 I7new I11 I8new k map5 map11
save('Results/HMM_functionally_different_output.mat');
